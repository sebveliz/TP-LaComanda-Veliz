...
public function CargarUno($request, $response, $args) 
{
    ...
    if(isset($body->codigoPedido) && isset($body->restaurante) && isset($body->mesa)
    && isset($body->cocinero)     && isset($body->mozo) && isset($body->experiencia))
    {

        if(is_numeric($body->restaurante) && is_numeric($body->mesa)
        && is_numeric($body->mozo)&& is_numeric($body->cocinero))
        {
            ...
            if($respuesta!=null)
            {
                ...
                $mesa= mesas::traerUnaMesa($respuesta->mesa);
