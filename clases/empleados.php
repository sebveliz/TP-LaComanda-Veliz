...
public function GuardarIngreso($datos,$empleado)
{
    ...
    $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
    $consulta =$objetoAccesoDato->RetornarConsulta("INSERT into login (nombre,idEmpleado,tipo,metodo,ruta,fechaYHora)values(:nombre,:idEmpleado,:tipo,:metodo,:ruta,:fechaYHora)");
    $retorno =  $consulta->execute(array($nombre,$id,$tipo,$metodo,$ruta, $fechaYHora));   
    //return $retorno;
...
public function TraerCantidadDeLogs()
{
    ...
    $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
    $consulta =$objetoAccesoDato->RetornarConsulta("select Tipo , count(*) as Ingresos from login group by tipo");
    $consulta->execute(); 
